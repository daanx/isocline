...
module System.Console.Isocline(
      ...
      -- * Low-level Terminal
      termInit, termInitStdErr,
      termDone,
      withTerm, withTermStdErr,
      termFlush,
      termWrite,
      termWriteLn,
      termColor,
...
foreign import ccall ic_init        :: CCBool -> IO ()
...
-- | Initialize Isocline using stderr for output (instead of stdout)
...
foreign import ccall ic_term_init_ex    :: CCBool -> IO ()
...
-- See also 'withTerm' and 'termInitEx'.
termInit :: IO ()
termInit
  = ic_term_init

-- | Initialize the terminal for the @term@ functions using stderr for output.
-- Does nothing on most platforms but on windows enables UTF8 output
-- and potentially enables virtual terminal processing.
-- See also 'withTermStdErr' and 'termInit'.
termInitStdErr :: IO ()
termInitStdErr
  = ic_term_init_ex (cbool True)
...
-- | Use the @term@ functions (brackets 'termInitStdErr' and 'termDone').
withTermStdErr :: IO a -> IO a
withTermStdErr action
  = bracket termInitStdErr (\() -> termDone) (\() -> action)
