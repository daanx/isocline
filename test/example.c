...
int main() 
{
  ...
  printf("terminal color bits: %d\n", rp_term_get_color_bits());

#define AT "@"

  rp_writeln("colors rgb:"); 
  rp_write("\n\n  // 9x9x17 colors");
  ...
  // shades
  for (int i = 0; i <= 64; i++) {
    rp_term_color(RP_RGBX((i==64 ? 255 : i*4), 0, 0)); rp_write((i%8==0 ? "#" : AT));
  }
  rp_writeln("");  
  for (int i = 0; i <= 64; i++) {
    rp_term_color(RP_RGBX(0, (i==64 ? 255 : i*4), 0)); rp_write((i%8==0 ? "#" : AT));
  }
  rp_writeln("");  
  for (int i = 0; i <= 64; i++) {
    rp_term_color(RP_RGBX(0, 0, (i==64 ? 255 : i*4))); rp_write((i%8==0 ? "#" : AT));
  }
  rp_writeln("");  
  for (int i = 0; i <= 64; i++) {
    int g = (i==64 ? 255 : i*4);
    rp_term_color(RP_RGBX(g, g, g)); rp_write((i%8==0 ? "#" : AT));
  }
  rp_writeln("");
  rp_term_reset();
  ...
  show_color(RP_ANSI_GREEN,"green");
